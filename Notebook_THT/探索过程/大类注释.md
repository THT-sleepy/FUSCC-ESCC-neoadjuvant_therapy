...
纹饰-dirtywork-一个亚群一个亚群来
思路是首先画含UMAP坐标的图，对每个群肉眼得到UMAP大致范围，然后取出亚群子集，再加上UMAP限定得到最终亚群
然后再合并，这样就可以去除那些散在的细胞群了

## 这里是去除乱跑细胞的dirty work(bushi)
```
...
plt.savefig("plots/umap_withaxis.png", dpi=300, bbox_inches="tight")
plt.show()
...
##B
mask = (umap1 >= -4) & (umap1 <= 8) & (umap2 >= 0) & (umap2 <= 6) & (adata.obs['first_round_celltype'] == "B")
adata_B = adata[mask, :].copy()

##Endo
mask = (umap1 >= 0) & (umap1 <= 8) & (umap2 >= 10) & (umap2 <= 15) & (adata.obs['first_round_celltype'] == "Endo")
adata_Endo = adata[mask, :].copy()

##上皮
mask = (umap1 >= 8) & (umap2 >= 0) & (umap2 <= 12.5) & (adata.obs['first_round_celltype'] == "Epi")
adata_Epi = adata[mask, :].copy()

##纤维
mask = (umap1 >= 4) & (umap1 <= 12) & (umap2 >= 12.5) & (umap2 <= 20) & (adata.obs['first_round_celltype'] == "Fib")
adata_Fib = adata[mask, :].copy()

##髓系
mask = (umap1 >= -12) & (umap1 <= 8) & (umap2 >= 2.5) & (umap2 <= 12.5) & (adata.obs['first_round_celltype'] == "Myeloid")
adata_Myeloid = adata[mask, :].copy()

##T
mask = (umap1 >= -4) & (umap1 <= 12) & (umap2 >= -15) & (umap2 <= 0) & (adata.obs['first_round_celltype'] == "T")
...
sc.pl.umap(adata_filtered,color="first_round_celltype",save="first_round_celltype_umapfiltered.png")
adata_filtered.write("RData/first_round_anno_umapfiltered.h5ad")

```
<img src="..\figures\umap_withaxis.png">
* 弄完之后的图
<img src="..\figures\umapfirst_round_celltype_umapfiltered.png">
* 这一小撮细胞要注意，之前发现是既表达B细胞marker又表达髓系marker(髓系亚群注释时要当心)
<img src="..\figures\first_round_celltype_标记.png"><br>

* 模仿chen的多marker区分图

## to do list
* 1 把亚群提出来重新标化，选高变基因，去批次，降维聚类(0.6-1.2)-running
* 1.5 把综述再改一改提交上去
* 2 把first_round_anno的代码整理出来，添上top10基因点-running
* 3 把inferCNV跑了，结果大致整理出来-不够亚群注释的时候如果又去了一些cluster就要重跑
* 4 中间的空闲时间可以查marker文献
* 5 空闲时间也可以研究下Figure1,Sup. Fig1需要的图片
这些事情搞好暂时应该就没啥事了,其它的后面再来慢慢研究吧
