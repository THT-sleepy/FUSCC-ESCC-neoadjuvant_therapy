...
adata.obs["myeloid_celltype"]=adata.obs.leiden_res0_8.map(cl_annotation)
...
<img src="..\figures\umap121celltypist_coarse_MonoMacro.png">
<img src="..\figures\umap121celltypist_fine_MonoMacro.png">

#### 把这个celltypist认为是B细胞的一小撮给去了
```
...
marker_genes = ["PTPRC","CD3E","CD79A","COL1A1",
"VWF","PECAM1","EPCAM","SFN","KRT5","LYZ","GCA","TPSAB1"]
sc.pl.umap(adata,color=marker_genes,vmin=0,vmax="p99",sort_order=False,cmap="Blues",save="121_MonoMacro_majormarker.png")

<img src="..\figures\umap121_MonoMacro_majormarker.png">
...
sc.pl.dotplot(
    ...
    var_names=marker_genes,
    standard_scale="var",
    save="MonoMacro_亚类marker.png"
)
```
<img src="..\figures\dotplot_MonoMacro_亚类marker.png">
3,4应该是Macro,其它的是Mono
...
sc.pl.rank_genes_groups(adata, n_genes=25, sharey=False,key="dea_leiden_res0_6",save="dea_MonoMacrores0_6.png")
sc.tl.filter_rank_genes_groups(
  adata,
  min_in_group_fraction=0.2,
  max_out_group_fraction=0.2,
...
sc.pl.rank_genes_groups_dotplot(
  ...
  save="121_res0_6_MonoMacro_top10gene.png"
)
```
