...
adata = sc.read("RData/first_round_anno.h5ad") #X是log1p_norm
...
def process_celltype(adata, celltype_name, save_dir="RData/", plot_prefix=""):
    ...
    """
    # 1. 提取目标细胞类型子集
    adata_sub = adata[adata.obs.major_celltype == celltype_name].copy()
    ...
    # 4. 循环运行不同min_dist的UMAP+Leiden聚类（0.1/0.2/0.4）
    for min_dist in [0.4]:
        # 复制数据避免相互干扰
        ...
        sc.tl.leiden(adata_md, key_added="leiden_res2", resolution=2)
        
        # 保存UMAP聚类图
        plot_path = f"{plot_prefix}{celltype_name}_umapmd{min_dist}_regressmtribo_cluster.png"
        sc.pl.umap(
            adata_md,
            color=["leiden_res0_6", "leiden_res0_8", "leiden_res1", "leiden_res1_2","leiden_res2"],
            legend_loc="on data",
            save=plot_path,
...
#celltypes_to_process = ["T", "B", "Epi", "Endo", "Fib", "Myeloid"]
celltypes_to_process = ["Epithelial cell", "Endothelial cell", "Fibrocyte", "Myeloid cell","B cell"] #"T&ILC cell",
