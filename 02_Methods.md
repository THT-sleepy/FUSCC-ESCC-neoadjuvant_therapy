...
?% (?/?) of T cells with high-quality TCR α-β pair were kept, resukting in ? clonotypes. The size of clonotypes ranged from 1 to ? or ? for CD4+ and CD8+ T cells,respectively, with ? clonotypes comprising more than one cell (Figures ? and ?). We calculated the STARTRAC-expa,STARTRAC-migr and STARTRAC-trans index with   the Startrac.run function from the R package Startrac (version 0.1.0)[citation]
